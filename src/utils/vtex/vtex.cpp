...
static bool LoadFaceFromPSD( IVTFTexture *pTexture, CUtlBuffer &psdBuffer, int z, int nFrame, int nFace, float flGamma, const VTexConfigInfo_t &info )
{
			...
			else
			{
				for(int x=0; x < pTexture->Width(); x++ )
				{
					for(int y=0; y < pTexture->Height(); y++ )
					{
						...
						uint8 nOrigAlpha = bmPsdData.GetBits()[3+4*(nOrig_x+nWidth*nOrig_y)];
						...
						for(int iy=-nSearchRad; iy <= nSearchRad; iy++ )
						{
							for(int ix=-nSearchRad; ix <= nSearchRad; ix++ )
							{
								...
								uint8 alphaValue = bmPsdData.GetBits()[nOffset];
						...
						float flOutDist = min( 0.5f, FLerp( 0.f, .5f, 0.f, flMaxRad, flClosest_Dist ) );
						if ( ! bInOrOut )
						{
							// negative distance
							flOutDist = -flOutDist;
	...
	else
	{
		...
		if ( ok )
		{
			memcpy( pDestBits, bmPsdData.GetBits(), bmPsdData.Width()* bmPsdData.Height()* ImageLoader::SizeInBytes( bmPsdData.Format()) );
		}
		return ok;
	}
}
...
class CVTex : public CBaseAppSystem< IVTex >, public ILaunchableDLL
{
public:
