...
bool CResponseSystem::FindBestResponse( const CriteriaSet& set, CRR_Response& response, IResponseFilter *pFilter )
{
	...
	if ( m_RulePartitions.IsValid( bestRule ) )
	{
		...
		iApplyContext = r->m_iContextFlags;

		response.SetMatchScore(scoreOfBestRule);
		valid = true;
	}

	response.Init( responseType, responseName, rp, ruleName, context, iApplyContext);
...
void ResponseRules::CResponseSystem::ParseRule_ApplyContextToSelf(Rule& newRule)
{
	newRule.m_iContextFlags |= APPLYCONTEXT_SELF;
}

void ResponseRules::CResponseSystem::ParseRule_ApplyContextToSquad(Rule& newRule)
{
	newRule.m_iContextFlags |= APPLYCONTEXT_SQUAD;
}

void ResponseRules::CResponseSystem::ParseRule_ApplyContextToEnemy(Rule& newRule)
{
...
void CResponseSystem::BuildDispatchTables()
{
	...
	m_RuleDispatch.Insert(RR_HASH("applycontexttoself"), &CResponseSystem::ParseRule_ApplyContextToSelf);
