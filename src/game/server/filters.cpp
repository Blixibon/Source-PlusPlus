...
public:
	...
	bool PassesFilterImpl(CBaseEntity* pCaller, CBaseEntity* pEntity)
	{
		...
		if (m_bFull)
		{
			if (pEntity->IsNPC() && pEntity->MyNPCPointer()->GetExpresser())
			{
				CAI_Expresser* pExpresser = pEntity->MyNPCPointer()->GetExpresser();
				pExpresser->GatherCriteria(&set, "", nullptr);
			}
			else
			{
				...
				pEntity->ModifyOrAppendDerivedCriteria(set);
			}
		}
		else
		{
