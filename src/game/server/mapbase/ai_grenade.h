...
public:
	...
	// Classname of an entity derived from CGrenadeFrag
...
void CAI_GrenadeUser<BASE_NPC>::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == COMBINE_AE_GREN_TOSS )
	{
		...
		if( m_NPCState == NPC_STATE_SCRIPT )
		{
			...
			CBaseEntity *pGrenade = Fraggrenade_Create( vecStart, vec3_angle, vecThrow, vecSpin, this, COMBINE_GRENADE_TIMER, !IsPlayerAlly(), GetGrenadeClassThrown());
			m_OnThrowGrenade.Set(pGrenade, pGrenade, this);
		}
		else
		{
			// Use the Velocity that AI gave us.
			CBaseEntity *pGrenade = Fraggrenade_Create( vecStart, vec3_angle, m_vecTossVelocity, vecSpin, this, COMBINE_GRENADE_TIMER, !IsPlayerAlly(), GetGrenadeClassThrown());
			m_OnThrowGrenade.Set(pGrenade, pGrenade, this);
...
bool CAI_GrenadeUser<BASE_NPC>::CanAltFireEnemy( bool bUseFreeKnowledge )
{
	...
	if (!GetActiveWeapon()->NPC_CanFireSecondary())
		return false;
