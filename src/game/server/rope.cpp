...
CRopeKeyframe::CRopeKeyframe()
{
	...
	m_flScrollSpeed = 0;
	m_RopeFlags.Set(ROPE_SIMULATE | ROPE_INITIAL_HANG);
...
{
	int newFlags = m_RopeFlags.Get();
	if (bAttach)
		newFlags |= ROPE_PLAYER_WPN_ATTACH;
	else
		newFlags &= ~ROPE_PLAYER_WPN_ATTACH;

	if (newFlags != m_RopeFlags.Get())
	{
		m_RopeFlags.GetForModify() = newFlags;
	}
}
...
)
{
	...
	pRet->m_RopeFlags.GetForModify() &= ~ROPE_INITIAL_HANG;
...
void CRopeKeyframe::ActivateStartDirectionConstraints(bool bEnable)
{
	...
	{
		m_fLockedPoints.GetForModify() &= ~((int)ROPE_LOCK_START_DIRECTION);
	}
}
...
void CRopeKeyframe::ActivateEndDirectionConstraints(bool bEnable)
{
	...
	{
		m_fLockedPoints.GetForModify() &= ~((int)ROPE_LOCK_END_DIRECTION);
	}
}
...
{
	if (iPt == 0)
		return GetEndPointPos2(m_hStartPoint.Get(), m_iStartAttachment, v);
	else
		return GetEndPointPos2(m_hEndPoint.Get(), m_iEndAttachment, v);
}
...
void CRopeKeyframe::NotifyPositionChanged(CBaseEntity *pEntity)
{
	...
	if ((m_RopeFlags.Get() & ROPE_RESIZE) && ents[0] && ents[0]->edict() && ents[1] && ents[1]->edict())
	{
		int len = (int)(ents[0]->GetAbsOrigin() - ents[1]->GetAbsOrigin()).Length() + m_Slack;
		if (len != m_RopeLength.Get())
		{
			m_RopeLength.GetForModify() = len;
		}
	}
...
int CRopeKeyframe::OnTakeDamage(const CTakeDamageInfo &info)
{
	...
	if (!(m_RopeFlags.Get() & ROPE_BREAKABLE))
		return false;
...
void CRopeKeyframe::DetachPoint(int iPoint)
{
	...
	m_fLockedPoints.GetForModify() &= ~(1 << iPoint);
}


void CRopeKeyframe::EnableCollision()
{
	if (!(m_RopeFlags.Get() & ROPE_COLLIDE))
	{
...
void CRopeKeyframe::EnableWind(bool bEnable)
{
	...
	if ((m_RopeFlags.Get() & ROPE_NO_WIND) != flag)
	{
		m_RopeFlags.GetForModify() |= flag;
	}
}
...
bool CRopeKeyframe::KeyValue(const char *szKeyName, const char *szValue)
{
	...
	else if (stricmp(szKeyName, "Type") == 0)
	{
		...
		if (iType == 0)
			m_nSegments.GetForModify() = ROPE_MAX_SEGMENTS;
		else if (iType == 1)
			m_nSegments.GetForModify() = ROPE_TYPE1_NUMSEGMENTS;
		else
			m_nSegments.GetForModify() = ROPE_TYPE2_NUMSEGMENTS;
	}
	...
	else if (stricmp(szKeyName, "NoWind") == 0)
	{
		if (atoi(szValue) == 1)
		{
			m_RopeFlags.GetForModify() |= ROPE_NO_WIND;
		}
	}
...
{
	m_flScrollSpeed.GetForModify() = inputdata.value.Float();
}
