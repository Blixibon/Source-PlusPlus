...
public:
    ...
    virtual CBasePlayer *GetPlayer() const
    {
        return m_pPlayer;
    }
...
public:
    ...
    static CBasePlayer *GetClosestPlayer(const Vector &vecPosition, float *distance = NULL, CBasePlayer *pIgnore = NULL, int team = NULL);
    static bool IsSpotOccupied(const Vector &vecPosition, CBasePlayer *pIgnore = NULL, float closeRange = 75.0f, int avoidTeam = NULL);

    static CBasePlayer *GetClosestPlayerByClass(const Vector &vecPosition, Class_T classify, float *distance = NULL, CBasePlayer *pIgnore = NULL);
    static bool IsSpotOccupiedByClass(const Vector &vecPosition, Class_T classify, CBasePlayer *pIgnore = NULL, float closeRange = 75.0f);

    static bool IsCrossingLineOfFire(const Vector &vecStart, const Vector &vecFinish, CBasePlayer *pIgnore = NULL, int ignoreTeam = NULL);
...
{
public:
    CollectHidingSpotsFunctor(CBasePlayer *me, const Vector &origin, float range, int flags, Place place = UNDEFINED_PLACE) : m_origin(origin)
    {
        m_me = me;
        m_count = 0;
        m_range = range;
