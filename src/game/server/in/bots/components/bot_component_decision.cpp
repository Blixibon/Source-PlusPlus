...
void CBotDecision::PerformSensing()
{
	...
	if (ShouldOnlyFeelPlayers()) {
		...
		for (int i = 1; i <= gpGlobals->maxClients; ++i) {
			CBasePlayer *pPlayer = ToInPlayer(i);
...
//================================================================================
bool CBotDecision::ShouldHelpDejectedFriend(CBasePlayer *pFriend) const
{
...
//================================================================================
CBasePlayer *CBotDecision::GetClosestDejectedFriend(bool prioritizeHumans, float *distance) const
{
	VPROF_BUDGET("CBotDecision::GetClosestDejectedFriend", VPROF_BUDGETGROUP_BOTS);

	float closest = MAX_TRACE_LENGTH;
	CBasePlayer *closestFriend = NULL;

	for (int it = 1; it <= gpGlobals->maxClients; ++it) {
		CBasePlayer *pPlayer = ToInPlayer(it);
...
bool CBotDecision::IsDangerousEnemy(CBaseEntity *pEnemy) const
{
	...
	if (pEnemy->IsPlayer()) {
		CBasePlayer *pPlayer = ToInPlayer(pEnemy);
...
bool CBotDecision::IsInFieldOfView(const Vector & pos)
{
	...
	return PointWithinViewAngle(GetHost()->EyePosition(), pos, view, cosf(0.5f * GetHost()->GetFOV() * M_PI / 180.0f));
}
