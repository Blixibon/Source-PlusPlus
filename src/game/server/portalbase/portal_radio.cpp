...
public:
	...
	virtual void	DissolveSound(CBaseAnimating* pDissolvingProp);
...
void CPortal_Dinosaur::Activate( void )
{
	...
	if (pAchievementMgr)
	{
		CBaseAchievement* pTransmissionRecvd = pAchievementMgr->GetAchievementByID(ACHIEVEMENT_PORTAL_TRANSMISSION_RECEIVED);
...
void CPortal_Dinosaur::DissolveSound(CBaseAnimating* pDissolvingProp)
{
	pDissolvingProp->EmitSound("UpdateItem.Fizzle");
}
