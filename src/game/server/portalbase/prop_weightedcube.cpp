...
const char* CUBE_MODEL = "models/props/portal2/metal_box.mdl";
...
const char* CUBE_FX_FIZZLER_MODEL = "models/props/portal2/metal_box_fx_fizzler.mdl";
...
class CCubeDissolveModel : public CBaseProp
{
	DECLARE_CLASS(CCubeDissolveModel, CBaseProp);
	DECLARE_SERVERCLASS();
public:
	CCubeDissolveModel()
	{
	...
	void Spawn()
	{
		SetModelName(AllocPooledString(CUBE_FX_FIZZLER_MODEL));
		BaseClass::Spawn();
...
void CPropWeightedCube::UpdateSchrodingerSound( void )
{
	...
	float fDist = m_hSchrodingerTwin->WorldSpaceCenter().DistTo(WorldSpaceCenter());
...
void CPropWeightedCube::StartTouch( CBaseEntity *pOther )
{
	if( m_bMovementDisabled )
	{
		if( pOther->IsPlayer() )
		{
			...
			float flCubeDirDot = DotProduct(vecForward.Normalized(), vecPlayerForward );
...
void CPropWeightedCube::CleanserDissolve(CBaseEntity* pActivator)
{
	if (IsDissolving())
	{
		return;
	...
	if (m_nCubeType == CUBE_STANDARD || m_nCubeType == CUBE_COMPANION)
	{
		Vector vOldVel;
		...
		CCubeDissolveModel* pDisolvingAnimating = static_cast<CCubeDissolveModel*> (CreateEntityByName("cube_dissolveanim"));
		if (pDisolvingAnimating)
		{
			...
			pDisolvingAnimating->m_nSkin = (m_nCubeType == CUBE_COMPANION) ? 1 : 0;
	...
	{
		BaseClass::CleanserDissolve(pActivator);
	}
}
