...
class CPropWeightedCube : public CPhysicsProp, public IPropWeightedCubeAutoList
{
public:
	...
	// Do the dissolve effects as if we just passed through a trigger_portal_cleanser
	virtual void	CleanserDissolve(CBaseEntity* pActivator);

protected:
	virtual void	OnFizzled(CBaseEntity* pActivator)
	{
		...
		if (V_stricmp(STRING(gpGlobals->mapname), "mp_coop_paint_red_racer") == 0)
		{
			...
			if (pEntity)
			{
				pEntity->RunScript("CoopCubeFizzle()", "CPropWeightedCube::OnFizzled");
			}

		}

		BaseClass::OnFizzled(pActivator);
	}
private:
