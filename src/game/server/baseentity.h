...
public:
	...
	void					TraceBleed( float flDamage, const Vector &vecDir, trace_t *ptr, int64 bitsDamageType );
...
inline const char* CBaseEntity::GetPreTemplateName()
{
	...
	V_strncpy(szStrippedName, STRING(m_iName.Get()), MIN((int)ARRAYSIZE(szStrippedName), pszDelimiter - STRING(m_iName.Get()) + 1));
