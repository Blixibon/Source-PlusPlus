...
void CAI_LeadBehavior::LeadPlayer( const AI_LeadArgs_t &leadArgs, CAI_LeadBehaviorHandler *pSink )
{
	...
	{
		pOuter->SetSpeechTarget( GetBestPlayer() );
	}
#endif
...
{
 	if ( !GetBestPlayer() /*|| AI_GetSinglePlayer()->IsDead()*/ )
		return false;
...
bool CAI_LeadBehavior::PlayerIsAheadOfMe( bool bForce )
{
	...
	if ( GetClosestPointOnRoute( GetBestPlayer()->GetAbsOrigin(), &vecClosestPoint ) )
	{
...
void CAI_LeadBehavior::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
		...
		{
			m_MoveMonitor.SetMark(GetBestPlayer(), 24 );
