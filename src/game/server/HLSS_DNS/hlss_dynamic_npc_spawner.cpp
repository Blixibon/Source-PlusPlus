...
BEGIN_DATADESC( CHLSS_Dynamic_NPC_Spawner )
	...
	DEFINE_FIELD(m_iNPCTeam,						FIELD_INTEGER),
...
void CHLSS_Dynamic_NPC_Spawner::Precache()
{
	...
	if ( m_iszTemplateData != NULL_STRING )
	{
		...
		if ( pEntity != NULL && pEntity->IsNPC())
		{
			...
			pEntity->MyNPCPointer()->UpdateTeam();
			m_iNPCTeam = pEntity->GetTeamNumber();
		
			if (FClassnameIs(pEntity, "npc_combinegunship") ||
				FClassnameIs(pEntity, "npc_strider"))
			{
				if (m_iNPCTeam == LazuulRules()->GetProtaganistTeam())
				{
					m_bRequiresHostileRPG = true;
				}
				else if (m_iNPCTeam == LazuulRules()->GetAntagonistTeam())
				{
...
void CHLSS_Dynamic_NPC_Spawner::DeathNotice( CBaseEntity *pVictim )
{
	...
	if (pDirector)
	{
		if (pVictim->GetTeamNumber() != LazuulRules()->GetProtaganistTeam() &&
		...
		{
			bool bAlly = pVictim->GetTeamNumber() == LazuulRules()->GetProtaganistTeam();
...
void CHLSS_Dynamic_NPC_Spawner::GetNextDistanceToSpawn()
{
	...
	int disposition = (m_iNPCTeam == LazuulRules()->GetProtaganistTeam()) ? D_LI : D_HT;
