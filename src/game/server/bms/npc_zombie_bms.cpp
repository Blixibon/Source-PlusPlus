...
class CZombieBMSBase : public CAI_BlendingHost<CNPC_BaseZombie>
{
	DECLARE_DATADESC();
	DECLARE_CLASS(CZombieBMSBase, CAI_BlendingHost<CNPC_BaseZombie>);
...
enum
{
	COND_BMSZ_BLOCKED_BY_DOOR = LAST_BASE_ZOMBIE_CONDITION,
	COND_BMSZ_DOOR_OPENED,
	COND_BMSZ_ZOMBIE_CHARGE_TARGET_MOVED,
};
...
enum
{
	SCHED_BMSZ_ZOMBIE_BASH_DOOR = LAST_BASE_ZOMBIE_SCHEDULE,
	SCHED_BMSZ_ZOMBIE_WANDER_ANGRILY,
	SCHED_BMSZ_ZOMBIE_CHARGE_ENEMY,
	SCHED_BMSZ_ZOMBIE_FAIL,
};
...
enum
{
	TASK_BMSZ_ZOMBIE_EXPRESS_ANGER = LAST_BASE_ZOMBIE_TASK,
	TASK_BMSZ_ZOMBIE_YAW_TO_DOOR,
	TASK_BMSZ_ZOMBIE_ATTACK_DOOR,
	TASK_BMSZ_ZOMBIE_CHARGE_ENEMY,
};
...
// Purpose: 
//-----------------------------------------------------------------------------
void CZombieBMSBase::Precache(void)
{
	BaseClass::Precache();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CZombieBMSBase::Spawn(void)
{
	Precache();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CZombieBMSBase::PrescheduleThink(void)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CZombieBMSBase::SelectSchedule(void)
{
...
//-----------------------------------------------------------------------------
void CZombieBMSBase::FootstepSound(bool fRightFoot)
{
	if (fRightFoot)
	{
...
//-----------------------------------------------------------------------------
void CZombieBMSBase::FootscuffSound(bool fRightFoot)
{
	if (fRightFoot)
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CZombieBMSBase::PainSound(const CTakeDamageInfo &info)
{
...
//-----------------------------------------------------------------------------
void CZombieBMSBase::IdleSound(void)
{
...
//-----------------------------------------------------------------------------
const char *CZombieBMSBase::GetHeadcrabClassname(void)
{
	return "npc_headcrab";
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
const char *CZombieBMSBase::GetHeadcrabModel(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *CZombieBMSBase::GetLegsModel(void)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
const char *CZombieBMSBase::GetTorsoModel(void)
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CZombieBMSBase::SetZombieModel(void)
{
...
//---------------------------------------------------------
void CZombieBMSBase::MoanSound(envelopePoint_t *pEnvelope, int iEnvelopeSize)
{
	if (IsOnFire())
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CZombieBMSBase::ShouldBecomeTorso(const CTakeDamageInfo &info, float flDamageThreshold)
{
	if (IsSlumped())
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CZombieBMSBase::GatherConditions(void)
{
	BaseClass::GatherConditions();

	static int conditionsToClear[] =
	{
		COND_BMSZ_BLOCKED_BY_DOOR,
		COND_BMSZ_DOOR_OPENED,
		COND_BMSZ_ZOMBIE_CHARGE_TARGET_MOVED,
	};
	...
	{
		ClearCondition(COND_BMSZ_BLOCKED_BY_DOOR);
		if (m_hBlockingDoor != NULL)
		{
			SetCondition(COND_BMSZ_DOOR_OPENED);
			m_hBlockingDoor = NULL;
		}
	}
	else
		SetCondition(COND_BMSZ_BLOCKED_BY_DOOR);

	if (ConditionInterruptsCurSchedule(COND_BMSZ_ZOMBIE_CHARGE_TARGET_MOVED))
	{
		if (GetNavigator()->IsGoalActive())
		{
			...
			{
				SetCondition(COND_BMSZ_ZOMBIE_CHARGE_TARGET_MOVED);
			}
...
int CZombieBMSBase::SelectFailSchedule(int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode)
{
	if (HasCondition(COND_BMSZ_BLOCKED_BY_DOOR) && m_hBlockingDoor != NULL)
	{
		ClearCondition(COND_BMSZ_BLOCKED_BY_DOOR);
		if (m_NextTimeToStartDoorBash.Expired() && failedSchedule != SCHED_BMSZ_ZOMBIE_BASH_DOOR)
			return SCHED_BMSZ_ZOMBIE_BASH_DOOR;
		m_hBlockingDoor = NULL;
	}

	if (failedSchedule != SCHED_BMSZ_ZOMBIE_CHARGE_ENEMY &&
		IsPathTaskFailure(taskFailCode) &&
		random->RandomInt(1, 100) < 50)
	{
		return SCHED_BMSZ_ZOMBIE_CHARGE_ENEMY;
	}

	if (failedSchedule != SCHED_BMSZ_ZOMBIE_WANDER_ANGRILY &&
		(failedSchedule == SCHED_TAKE_COVER_FROM_ENEMY ||
			failedSchedule == SCHED_CHASE_ENEMY_FAILED))
	{
		return SCHED_BMSZ_ZOMBIE_WANDER_ANGRILY;
	}
...
Activity CZombieBMSBase::NPC_TranslateActivity(Activity newActivity)
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CZombieBMSBase::OnStateChange(NPC_STATE OldState, NPC_STATE NewState)
{
...
void CZombieBMSBase::StartTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_BMSZ_ZOMBIE_EXPRESS_ANGER:
	{
	...
	case TASK_BMSZ_ZOMBIE_YAW_TO_DOOR:
	{
	...
	case TASK_BMSZ_ZOMBIE_ATTACK_DOOR:
	{
	...
	case TASK_BMSZ_ZOMBIE_CHARGE_ENEMY:
	{
		if (!GetEnemy())
...
void CZombieBMSBase::RunTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_BMSZ_ZOMBIE_ATTACK_DOOR:
...
bool CZombieBMSBase::OnObstructingDoor(AILocalMoveGoal_t *pMoveGoal, CBaseDoor *pDoor,
...
Activity CZombieBMSBase::SelectDoorBash()
{
...
//---------------------------------------------------------
void CZombieBMSBase::Ignite(float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner)
{
	if (!IsOnFire() && IsAlive())
	{
...
//---------------------------------------------------------
void CZombieBMSBase::Extinguish()
{
	if (m_pMoanSound)
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CZombieBMSBase::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo)
{
#ifndef HL2_EPISODIC
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CZombieBMSBase::IsHeavyDamage(const CTakeDamageInfo &info)
{
#ifdef HL2_EPISODIC
...
bool CZombieBMSBase::IsSquashed(const CTakeDamageInfo &info)
{
	if (GetHealth() > 0)
	{
		return false;
...
//---------------------------------------------------------
//---------------------------------------------------------
void CZombieBMSBase::BuildScheduleTestBits(void)
{
...
class CNPC_ZombieScientist : public CZombieBMSBase
{
public:
	DECLARE_CLASS(CNPC_ZombieScientist, CZombieBMSBase);
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_ZombieScientist::Precache(void)
{
	BaseClass::Precache();

	PrecacheModel("models/zombies/zombie_sci.mdl");
	PrecacheModel("models/zombies/zombie_sci_torso.mdl");
	//PrecacheModel("models/zombie/classic_legs.mdl");
}
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *CNPC_ZombieScientist::GetLegsModel(void)
{
	return NULL;
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
const char *CNPC_ZombieScientist::GetTorsoModel(void)
{
	return "models/zombies/zombie_sci_torso.mdl";
}


//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_ZombieScientist::SetZombieModel(void)
{
	Hull_t lastHull = GetHullType();

	if (m_fIsTorso)
	{
		SetModel("models/zombies/zombie_sci_torso.mdl");
		SetHullType(HULL_TINY);
	}
	else
	{
		SetModel("models/zombies/zombie_sci.mdl");
		SetHullType(HULL_HUMAN);
	}
...
LINK_ENTITY_TO_CLASS(npc_zombie_scientist, CNPC_ZombieScientist);
LINK_ENTITY_TO_CLASS(npc_zombie_scientist_torso, CNPC_ZombieScientist);

class CNPC_ZombieGuard : public CZombieBMSBase
{
public:
	DECLARE_CLASS(CNPC_ZombieGuard, CZombieBMSBase);
	DECLARE_SERVERCLASS();
...
IMPLEMENT_SERVERCLASS_ST(CNPC_ZombieGuard, DT_ZombieGuard)
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_ZombieGuard::Precache(void)
{
	BaseClass::Precache();

	PrecacheModel("models/zombies/zombie_guard.mdl");
	PrecacheModel("models/zombies/zombie_guard_torso.mdl");
	//PrecacheModel("models/zombie/classic_legs.mdl");

	PrecacheParticleSystem("npc_zombie_security_mouth_fx");
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
const char *CNPC_ZombieGuard::GetLegsModel(void)
{
	return NULL;
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
const char *CNPC_ZombieGuard::GetTorsoModel(void)
{
	return "models/zombies/zombie_guard_torso.mdl";
}


//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_ZombieGuard::SetZombieModel(void)
{
	Hull_t lastHull = GetHullType();

	if (m_fIsTorso)
	{
		SetModel("models/zombies/zombie_guard_torso.mdl");
		SetHullType(HULL_TINY);
	}
	else
	{
		SetModel("models/zombies/zombie_guard.mdl");
		SetHullType(HULL_HUMAN);
	}
...
LINK_ENTITY_TO_CLASS(npc_zombie_security, CNPC_ZombieGuard);

class CNPC_ZombieGrunt : public CZombieBMSBase
{
public:
	DECLARE_CLASS(CNPC_ZombieGrunt, CZombieBMSBase);
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_ZombieGrunt::Precache(void)
{
	BaseClass::Precache();

	PrecacheModel("models/zombies/zombie_grunt.mdl");
	PrecacheModel("models/zombies/zombie_grunt_torso.mdl");
	//PrecacheModel("models/zombie/classic_legs.mdl");
}
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *CNPC_ZombieGrunt::GetLegsModel(void)
{
	return NULL;
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
const char *CNPC_ZombieGrunt::GetTorsoModel(void)
{
	return "models/zombies/zombie_grunt_torso.mdl";
}


//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_ZombieGrunt::SetZombieModel(void)
{
	Hull_t lastHull = GetHullType();

	if (m_fIsTorso)
	{
		SetModel("models/zombies/zombie_grunt_torso.mdl");
		SetHullType(HULL_TINY);
	}
	else
	{
		SetModel("models/zombies/zombie_grunt.mdl");
		SetHullType(HULL_HUMAN);
	}
...
AI_BEGIN_CUSTOM_NPC(npc_zombie_bms, CZombieBMSBase)

DECLARE_CONDITION(COND_BMSZ_BLOCKED_BY_DOOR)
DECLARE_CONDITION(COND_BMSZ_DOOR_OPENED)
DECLARE_CONDITION(COND_BMSZ_ZOMBIE_CHARGE_TARGET_MOVED)

DECLARE_TASK(TASK_BMSZ_ZOMBIE_EXPRESS_ANGER)
DECLARE_TASK(TASK_BMSZ_ZOMBIE_YAW_TO_DOOR)
DECLARE_TASK(TASK_BMSZ_ZOMBIE_ATTACK_DOOR)
DECLARE_TASK(TASK_BMSZ_ZOMBIE_CHARGE_ENEMY)
...
(
	...
	"	Tasks"
	"		TASK_BMSZ_ZOMBIE_CHARGE_ENEMY		0"
	"		TASK_WALK_PATH					0"
