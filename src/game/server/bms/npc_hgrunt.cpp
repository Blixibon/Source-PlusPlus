...
private:
	...
	class CCombineStandoffBehavior : public CAI_ComponentWithOuter<CNPC_Hgrunt, CAI_StandoffBehavior>
	{
		typedef CAI_ComponentWithOuter<CNPC_Hgrunt, CAI_StandoffBehavior> BaseClass;
...
//-----------------------------------------------------------------------------
void CNPC_Hgrunt::InputHitByBugbait(inputdata_t &inputdata)
{
...
void CNPC_Hgrunt::Precache()
{
	...
	PrecacheScriptSound("NPC_Grunt.WeaponBash");
...
void CNPC_Hgrunt::PostNPCInit()
{
	if (IsElite())
	{
		...
		// an AR2. 
		if (!GetActiveWeapon() || (!FClassnameIs(GetActiveWeapon(), "weapon_ar2") && !FClassnameIs(GetActiveWeapon(), "weapon_smg1")))
		{
			DevWarning("**Combine Elite Soldier MUST be equipped with AR2 or SMG1\n");
		}
	}
...
// 
//-----------------------------------------------------------------------------
bool CNPC_Hgrunt::ShouldMoveAndShoot()
{
...
// Chase the enemy, updating the target position as the player moves
//-----------------------------------------------------------------------------
void CNPC_Hgrunt::StartTaskChaseEnemyContinuously(const Task_t *pTask)
{
...
void CNPC_Hgrunt::RunTaskChaseEnemyContinuously(const Task_t *pTask)
{
	if (!GetNavigator()->IsGoalActive())
	{
		SetIdealActivity(GetStoppedActivity());
...
Activity CNPC_Hgrunt::NPC_TranslateActivity(Activity eNewActivity)
{
	...
	if (!GetActiveWeapon())
	{
		...
		else if (eNewActivity == ACT_WALK || eNewActivity == ACT_RUN)
...
int CNPC_Hgrunt::SelectSchedule(void)
{
	...
	if (m_NPCState != NPC_STATE_SCRIPT)
	{
		...
		if (m_NPCState != NPC_STATE_DEAD && m_NPCState != NPC_STATE_PRONE)
		{
			...
			if (HasCondition(COND_HEAR_DANGER))
			{
				...
				if (pSound)
				{
					if (pSound->m_iType & SOUND_DANGER)
					{
						...
						const char *pSentenceName = TLK_HG_COVER;
			...
			if (gpGlobals->curtime > m_flNextPainSoundTime && m_iHealth <= m_iMaxHealth * 0.5 && CanSpeak() && GetExpresser()->CanSpeakConcept(TLK_HG_MEDIC))
			{
				Speak(TLK_HG_MEDIC);
				//AI_Response *pResponse = SpeakFindResponse(TLK_HG_MEDIC);
				m_flNextPainSoundTime = GetExpresser()->GetTimeSpeechComplete() + 1.0f;
			}
		}
...
int CNPC_Hgrunt::SelectScheduleAttack()
{
	// Drop a grenade?
...
//=========================================================
//=========================================================
void CNPC_Hgrunt::OnStartSchedule(int scheduleType)
{
}
		...
		case COMBINE_AE_KICK:
		{
			...
			if (pBCC)
			{
				...
				if (!pBCC->DispatchInteraction(g_interactionCombineBash, NULL, this))
				{
					...
					EmitSound("NPC_Grunt.WeaponBash");
				}
			}
...
void CNPC_Hgrunt::PainSound(const CTakeDamageInfo &info)
{
	...
	{
		//const char *pSentenceName = TLK_HG_MEDIC;
...
void CNPC_Hgrunt::DeathSound(void)
{
	...
	if (GetParametersForSound("NPC_Grunt.Death", params, STRING(GetModelName())))
	{
		CPASAttenuationFilter filter(this, params.soundlevel);
		EmitSound_t es(params);
		if (GetSpecialDSP() != 0)
		{
			es.m_nSpecialDSP = GetSpecialDSP();
		}
...
void CNPC_Hgrunt::IdleSound(void)
{
	...
	if (g_fHGruntQuestion || random->RandomInt(0, 1))
	{
		...
		else
		{
			...
			{
			case 1: // check in
				if (Speak(TLK_HG_CLEAR))
				{
					g_fHGruntQuestion = 0;
				}
				break;
			case 2: // question 
				if (Speak(TLK_HG_ANSWER))
				{
					g_fHGruntQuestion = 0;
				}
				break;
...
NPC_STATE CNPC_Hgrunt::SelectIdealState(void)
{
	...
	default:
	{
		NPC_STATE NPCS_ret = BaseClass::SelectIdealState();
		if (NPCS_ret == NPC_STATE_COMBAT && m_NPCState < NPC_STATE_COMBAT)
		{
			Speak(TLK_STARTCOMBAT);
		}
		return NPCS_ret;
	}
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Hgrunt::IsUsingTacticalVariant(int variant)
{
...
"		TASK_HGRUNT_PLAY_SEQUENCE_FACE_ALTFIRE_TARGET		ACTIVITY:ACT_COMBINE_AR2_ALTFIRE"
""
"	Interrupts"
)
...
//	>> CNPC_Human_Grunt
//=========================================================
class CNPC_Human_Grunt : public CNPC_Hgrunt
{
	DECLARE_CLASS(CNPC_Human_Grunt, CNPC_Hgrunt);
#if HL2_EPISODIC
	DECLARE_DATADESC();
#endif
