...
namespace Mounter
{
	...
	static const char *s_pchSentenceFileHeader = "//This sentence file AUTOGENERATED by Petercov's Source-PlusPlus content mounter.\n//This file is regenerated at game start.\n\n";
	...
	{
		char dir[MAX_PATH];
		V_strncpy(dir, pszFileName, MAX_PATH);
		V_StripFilename(dir);
		pFileSystem->CreateDirHierarchy(dir, "MOD_WRITE");

		CUtlBuffer buf(0, 0, CUtlBuffer::TEXT_BUFFER);
		buf.Put(s_pchSentenceFileHeader, V_strlen(s_pchSentenceFileHeader));
		
		for (int i = g_SentenceFiles.Count()-1; i >= 0; i--)
		{
			const char *pszFile = g_SentenceFiles.Element(i);
			if (!pFileSystem->FileExists(pszFile))
				continue;

			CUtlBuffer fileBuf(0, 0, CUtlBuffer::TEXT_BUFFER);
			if (!pFileSystem->ReadFile(pszFile, NULL, fileBuf))
				continue;

			buf.Put(fileBuf.Base(), fileBuf.IsReadOnly() ? fileBuf.TellMaxPut() : fileBuf.TellPut());
		}

		if (!pFileSystem->WriteFile(pszFileName, "MOD_WRITE", buf))
			return false;
