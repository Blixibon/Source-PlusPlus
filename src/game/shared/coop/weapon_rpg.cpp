...
public:
	...
	int		ObjectCaps()
	{
		...
		if (!m_bSaved)
			iCaps = (iCaps & ~FCAP_ACROSS_TRANSITION) | FCAP_DONT_SAVE;
...
#ifdef CLIENT_DLL
RecvPropBool(RECVINFO(m_bSaved)),
#else
SendPropBool(SENDINFO(m_bSaved)),
#endif
END_NETWORK_TABLE()
...
#define	RPG_GUIDE_ATTACHMENT		3
#define	RPG_GUIDE_TARGET_ATTACHMENT	4
...
BEGIN_DATADESC( CLaserDot )
	...
	DEFINE_FIELD(m_bSaved, FIELD_BOOLEAN),
