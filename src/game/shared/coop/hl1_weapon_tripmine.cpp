...
class CWeaponTripMine : public CCoopHL1CombatWeapon
{
	DECLARE_CLASS( CWeaponTripMine, CCoopHL1CombatWeapon);
	DECLARE_PREDICTABLE();
public:

	CWeaponTripMine( void );

	virtual int GetWeaponID(void) const { return HLSS_WEAPON_ID_TRIPMINE; }
...
private:
	CNetworkVar(int, m_iGroundIndex);
	CNetworkVar(int, m_iPickedUpIndex);
};
...
#ifdef CLIENT_DLL
RecvPropInt(RECVINFO(m_iGroundIndex)),
RecvPropInt(RECVINFO(m_iPickedUpIndex)),
#else
SendPropModelIndex(SENDINFO(m_iGroundIndex)),
SendPropModelIndex(SENDINFO(m_iPickedUpIndex)),
#endif
END_NETWORK_TABLE()
