...
class CWeaponEgon : public CCoopHL1CombatWeapon
{
	DECLARE_CLASS( CWeaponEgon, CCoopHL1CombatWeapon);
public:
	...
	virtual int GetWeaponID(void) const { return HLSS_WEAPON_ID_EGON; }
...
private:
	...
	float				m_flDmgTime;
	CNetworkHandle(CSprite, m_hSprite);
...
#ifdef CLIENT_DLL
RecvPropEHandle(RECVINFO(m_hSprite)),
RecvPropEHandle(RECVINFO(m_hBeam)),
RecvPropEHandle(RECVINFO(m_hNoise)),
#else
SendPropEHandle(SENDINFO(m_hSprite)),
...
#ifdef CLIENT_DLL
DEFINE_PRED_FIELD(m_hSprite, FIELD_EHANDLE, FTYPEDESC_INSENDTABLE),
DEFINE_PRED_FIELD(m_hBeam, FIELD_EHANDLE, FTYPEDESC_INSENDTABLE),
DEFINE_PRED_FIELD(m_hNoise, FIELD_EHANDLE, FTYPEDESC_INSENDTABLE),
#endif
END_PREDICTION_DATA()
...
void CWeaponEgon::CreateEffect( void )
{
	...
	m_hBeam = BEAM_CREATE_PREDICTABLE_PERSIST(EGON_BEAM_SPRITE, 3.5, pPlayer);
	m_hBeam->PointEntInit(GetAbsOrigin(), this);
	m_hBeam->SetBeamFlags(FBEAM_SINENOISE);
	m_hBeam->SetEndAttachment(1);
	m_hBeam->SetOwnerEntity(pPlayer);
	...
	m_hNoise = BEAM_CREATE_PREDICTABLE_PERSIST(EGON_BEAM_SPRITE, 5.0, pPlayer);
	m_hNoise->PointEntInit(GetAbsOrigin(), this);
	m_hNoise->SetEndAttachment(1);
	m_hNoise->SetOwnerEntity(pPlayer);
	m_hNoise->SetScrollRate(25);
	...
	m_hSprite = SPRITE_CREATE_PREDICTABLE(EGON_FLARE_SPRITE, GetAbsOrigin(), false);
	m_hSprite->SetScale(1.0);
	m_hSprite->SetTransparency(kRenderGlow, 255, 255, 255, 255, kRenderFxNoDissipation);
	m_hSprite->SetOwnerEntity(pPlayer);
...
void CWeaponEgon::DestroyEffect( void )
{
	...
	if ( m_hSprite )
	{
		m_hSprite->SetPredictionEligible(false);
	...
	if (prediction->IsFirstTimePredicted())
	{
		if (m_hBeam)
		{
			m_hBeam->AddEffects(EF_NODRAW);
		}
		if (m_hNoise)
		{
			m_hNoise->AddEffects(EF_NODRAW);
		}
		if (m_hSprite)
		{
			m_hSprite->Expand(10, 500);
