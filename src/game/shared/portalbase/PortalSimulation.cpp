...
static void CarveEntity(PS_PlacementData_t& PlacementData, PS_SD_Dynamic_CarvedEntities_t& CarvedEntities, PS_SD_Dynamic_CarvedEntities_CarvedEntity_t& CarvedRepresentation)
{
	...
	if (solidType == SOLID_VPHYSICS || solidType == SOLID_BSP)
	{
...
void CPortalSimulator::CreateLocalCollision(void)
{
	...
	if (portal_clone_displacements.GetBool())
	{
		...
		CPhysCollide* pBaseDispColl = enginetrace->GetCollidableFromDisplacementsInAABB(m_InternalData.Placement.vecCurAABBMins, m_InternalData.Placement.vecCurAABBMaxs);
		if (pBaseDispColl)
		{
			ICollisionQuery* pQuery = physcollision->CreateQueryModel(pBaseDispColl);
			if (pQuery)
			{
				...
				for (int i = 0; (i != pQuery->ConvexCount()) && (iTriCount < 65535); ++i)
				{
					//virtualmeshlist_t* pMesh = &DisplacementMeshes[i];

					for (int j = 0; j < pQuery->TriangleCount(i); j += 1)
					{
						Vector points[3];
							...
							if (iFront != 0)
							{
								if (iBack != 0)
								{
									...
									if (iBack == 2)
									{
										...
										physcollision->PolysoupAddTriangle(pDispCollideSoup, points[0], points[1], points[2], pQuery->GetTriangleMaterialIndex(i, j));
										++iTriCount;
									}
									else
									{
			...
			physcollision->DestroyCollide(pBaseDispColl);
		}
			...
			for (int i = StaticProps.Count(); --i >= 0; )
			{
				...
				if (indices.iNumPolyhedrons != 0)
				{
					...
					if (bIsStudioModel)
					{
						...
						NewEntry.iTraceSurfaceProps = pStudioHdr->surfacepropindex;
					}
					else
					{
