...
#else
	baseFilter.AddClassnameToIgnore("class C_Prop_Portal");
...
#else
		tr.m_pEnt->EstimateAbsVelocity(vVelocityCheck);
		QAngle tmp = tr.m_pEnt->GetLocalAngularVelocity();
		QAngleToAngularImpulse(tmp, vAngularImpulseCheck);
#endif // !CLIENT_DLL
