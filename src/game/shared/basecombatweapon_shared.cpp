...
{ 
	//Adrian: Oh man...
#if !defined( CLIENT_DLL ) && (defined( HL2MP ) || defined( PORTAL ) || defined( HL2_LAZUL ))
...
void CBaseCombatWeapon::TertiaryAttack(void)
{
	CBasePlayer *pPlayer = ToBasePlayer(GetOwner());
...
{
#ifdef CLIENT_DLL
	if (g_pGameRules->IsMultiplayer() && !prediction->IsFirstTimePredicted())
		return;
#endif
...
BEGIN_PREDICTION_DATA( CBaseCombatWeapon )
	...
	DEFINE_FIELD(m_bHasBeenDeployed, FIELD_BOOLEAN),
...
BEGIN_NETWORK_TABLE(CBaseCombatWeapon, DT_BaseCombatWeapon)
	...
	SendPropBool(SENDINFO(m_bHasBeenDeployed)),
#else
	...
	RecvPropBool(RECVINFO(m_bHasBeenDeployed)),
#endif
END_NETWORK_TABLE()
