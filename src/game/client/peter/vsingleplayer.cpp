...
//-----------------------------------------------------------------------------
CNewGameDialog::CNewGameDialog(vgui::Panel *parent, const char *name, const char *game, bool bCommentaryMode) : BaseClass(parent, name)
{
	...
	if ( IsPC() )
	{
		...
		Q_snprintf(szFile, sizeof(szFile), "cfg/%s/chapter*.cfg", game);
		...
		while ( fileName && chapterIndex < MAX_CHAPTERS )
		{
			...
			Q_snprintf( szFullFileName, sizeof(szFullFileName), "cfg/%s/%s", game, fileName );
	...
	for (int i = 0; i < chapterIndex; i++)
	{
		...
		const char *pGameDir = game;

		char chapterName[64];
		Q_snprintf(chapterName, sizeof(chapterName), "#%s_Chapter%s_Title", pGameDir, chapterID);

		Q_snprintf( szFullFileName, sizeof( szFullFileName ), "%s/%s", game, fileName );
		CGameChapterPanel *chapterPanel = SETUP_PANEL( new CGameChapterPanel( this, chapterID, chapterName, i, chapterID, szFullFileName, m_bCommentaryMode ) );
...
void CNewGameDialog::OnCommand( const char *command )
{
	...
	else if (!stricmp(command, "Back"))
	{
		...
		OnClose();
		CreateNewGameDialog(m_bCommentaryMode);
	}
	else
	{
