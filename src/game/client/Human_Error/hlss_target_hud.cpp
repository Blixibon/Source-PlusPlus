...
void HLSS_DrawTargetHud(Vector vecOrigin, C_BaseEntity* pPlayer, C_BaseEntity* pTarget, int iEnemyType, vgui::HFont hFont, int viewX, int viewY, int viewWidth, int viewHeight, const VMatrix& matWorldToScreen)
{
	...
	float x = (viewWidth / 2) + viewX;
	float y = (viewHeight / 2) + viewY;
	...
	FrustumTransform(matWorldToScreen, vecTest, screen);
	float x1 = x + (0.5 * screen[0] * viewWidth + 0.5);
	float y1 = y - (0.5 * screen[1] * viewHeight + 0.5);
	...
	FrustumTransform(matWorldToScreen, vecTest, screen);
	float x4 = x + (0.5 * screen[0] * viewWidth + 0.5);
	float y4 = y - (0.5 * screen[1] * viewHeight + 0.5);
	...
	if (flScaleUpper >= 0.16f && flScaleLower >= 0.16f)
	{
		...
		if (iEnemyType > 0)
		{
			...
			float ypos = vgui::surface()->GetFontTall(hFont) * 1.1f;
			float xpos = (vgui::surface()->GetCharacterWidth(hFont, '0') * length) + (flLowerX * flScaleLower);
			...
			if ((x4 - x1) >= xpos && (y4 - y1) >= ((2.0f * ypos) + (flUpperY * flScaleUpper)))
			{
				...
				x1 += vgui::surface()->GetCharacterWidth(hFont, '0');
...
void HLSS_DrawTargetHud(Vector vecOrigin, C_BasePlayer* pPlayer, C_BaseEntity* pTarget, int iEnemyType)
{
	...
	HLSS_DrawTargetHud(vecOrigin, pPlayer, pTarget, iEnemyType, m_hFont, 0, 0, ScreenWidth(), ScreenHeight(), engine->WorldToScreenMatrix());
}