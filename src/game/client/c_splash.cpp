...
void C_Splash::RenderParticles(CParticleRenderIterator* pIterator)
{
	...
	const SimpleParticle* pSimpleParticle = (const SimpleParticle*)pIterator->GetFirst();
	while (pSimpleParticle)
	{
		// Calculate color
		...
		pSimpleParticle = (const SimpleParticle*)pIterator->GetNext(sortKey);
	}
}
...
void C_Splash::SimulateParticles(CParticleSimulateIterator* pIterator)
{
	...
	while (pParticle)
	{
		...
		if (pParticle->m_flLifetime >= m_flParticleLifetime)
		{
		...
		else
		{
			...
			if (m_ParticleCollision.MoveParticle(pParticle->m_Pos, pParticle->m_vecVelocity, NULL, timeDelta, &trace))
			{
				...
				if (DotProduct(trace.plane.normal, Vector(0, 0, 1)) > 0.8)
				{
